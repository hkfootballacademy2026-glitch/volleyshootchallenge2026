...
type RawFrame = { left: RawFoot; right: RawFoot; frames: number; resized: number; ts: number; lastError?: string };
...
export function useLightweightFootDetection(enabled: boolean, cameraPosition: CameraPosition = "front") {
  ...
  const handleFrame = useCallback((event: RawFrame) => {
    ...
    const toFoot = (raw: RawFoot, side: "L" | "R"): { foot: FootPoint | null; rawSpeed: number } => {
      ...
      const velocity = plausible ? calcFootVelocity(x, y, prev, now) : { speed: 0, velX: 0, velY: 0 };
    ...
    if (event.left || event.right) setLastDetectionAt(now);
  ...
  const frameProcessor = useFrameProcessor(
    (frame) => {
      ...
      try {
        ...
        const ts = Date.now();
        handleFrameOnJS({ left, right, frames: 1, resized: 1, ts, lastError: "" });
      } catch (error) {
        const ts = Date.now();
        handleFrameOnJS({ left: null, right: null, frames: 1, resized: 0, ts, lastError: String(error) });
      }
    },
